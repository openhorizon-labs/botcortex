...
| Hand-written rules, no memory | 3.08 | 103/117 | +2.54 | 12/117 |
| Target-body memory, other faults only | 2.75 | 105/117 | +2.91 | 21/117 |
| **Cross-body memory, contract signatures, partial matches allowed** | **2.83** | **110/117** | **+3.07** | **9/117** |
| Cross-body memory, exact match or abstain | 2.99 | 103/117 | +2.62 | 11/117 |
| Cross-body memory, trace text (H2 control) | 3.01 | 110/117 | +2.90 | 16/117 |
| Cross-body memory plus the rule probe, probe at unit cost / quarter cost | 2.53 / 2.15 | 109/117 | +3.30 / +3.66 | 16/117 |

What changed against the pilot. H1 holds at four tasks and four fault groups, with the fewest negative cases of any memory arm. The abstention result **reversed**: exact-match-or-abstain, the pilot's best arm, is now the worst memory arm, because with twelve repairs the prior order a silent memory falls back to is long; whether abstention pays depends on the size of the repair catalog, and the paper will report that dependence rather than a fixed threshold. The H2 gap stays small (2.83 against 3.01) for the same reason as before. Probes pay only when cheap — at a quarter of an attempt the probing memory arm is the best in the study — but the new fault pairs need them: the fixture check is what separates a frame offset from a biased estimator (memory alone spends 4.5 attempts on the frame offset, the rules that ask the fixture 2.0), and random probing at the same trigger is consistently worse than the rule, so the choice of measurement carries the value. One fault is memory's blind spot on every body: a carry too low leaves the signature a wrong object, a swapped identity and weak servos leave elsewhere. The rules were fixed for it (a collision with the block still where it was is a carry too low; the servo probe rules out weak gains) and recover in 1.0 attempts; cross-body memory still exhausts its budget on all four cases (no memory: 2.0), because the records that share the signature outnumber the four low-carry records and carry the wrong repair. The fact that would separate them — which object was in the jaws when the arm stopped — is not in the evidence packet yet; adding it is the next signature change. A memory is only as discriminating as the evidence it is keyed on.
