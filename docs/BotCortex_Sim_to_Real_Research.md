...
**Simulation pilot — 15 September 2026:** the thin slice (§15) ran end to end in simulation on four bodies. Cross-body repair memory cut attempts-to-recovery from 4.70 to 1.85 on a held-out body and beat hand-written rules (2.67); negative transfer was measured and all but one case removed by letting retrieval abstain on partial matches (1.59 attempts, 1 of 27 worse than no memory); same-body memory about other faults was harmful (6.52). Numbers are from the re-scoring after the RoArm model was rebuilt on the vendor's geometry. Details in §2 under *Simulation pilot* and in `botcortex-runtime/docs/research/slice_pilot.md`. Not a hardware result.

**Widened benchmark — 16–17 September 2026:** the slice was widened to six bodies, four task families, four fault groups, twelve repairs and five probes (§2, *Widened benchmark*; report `botcortex-runtime/docs/research/benchmark_v2.md`). **189 scored cases**, re-measured from scratch on 17 September after a workcell layout error was found and fixed (the insertion pocket overlapped the tray on five bodies and hung off the table on the ViperX). Cross-body repair memory recovers in 2.84 attempts against 5.00 with no memory and 3.19 for hand-written rules, with the fewest negative-transfer cases of any arm (13 of 189). The sharpest result is the split between the four bodies the rules were written against and the two brought up afterwards: the rules lose 1.39 attempts on the unseen bodies and memory 0.70, on cases that are equally hard with no memory. A prediction the four-body run made — that more bodies would let memory learn a carry too low — was tested by adding those bodies and mostly failed; the cause is signature aliasing across gripper types, not data sparsity. Not a hardware result.
...
The milestone in §15 was built and run in the runtime repository (`botcortex/research/`, `docs/research/slice_pilot.md`). Four simulated bodies — OpenArm v1, a RoArm-M2 model on the vendor's URDF frames and MIT-licensed meshes, and the Menagerie Panda and xArm7 — run one unchanged pick-and-place program through one descriptor-driven runtime. Eight controlled faults (four program, four perception) each have one inverse repair; an independent verifier grades the task from simulator state the program never reads; every (body, fault, repair) trial is simulated once into a frozen outcome table, and every selector is scored against that table. Retrieval for the target body excludes the target itself; the target-only arm is leave-one-fault-out.

| Arm | Mean repairs to verified recovery (27 cases, budget 8) | Saved vs no memory | Worse than no memory | First pick in the right family |
| --- | --- | --- | --- | --- |
| No memory / fixed strategy | 4.70 | 0 | 0/27 | 13/27 |
| Hand-written rules, no memory | 2.67 | +2.04 | 1/27 | 25/27 |
| Target-body memory, other faults only | 6.52 | −1.81 | 19/27 | 15/27 |
| Cross-body memory, contract signatures, partial matches allowed | 1.85 | +2.85 | 1/27 | 25/27 |
| **Cross-body memory, contract signatures, exact match or abstain** | **1.59** | **+3.11** | 1/27 | 25/27 |
| Cross-body memory, exact, plus the rule probe at a quarter cost | 1.52 | +3.19 | 1/27 | 25/27 |
| Cross-body memory, trace text (H2 control) | 2.22 | +2.48 | 1/27 | 19/27 |

What the pilot supports, and only in simulation: repair knowledge from other bodies reduces recovery attempts on an unseen body (H1 direction), beating the rule baseline; transfer is perfect where a fault leaves the same contract signature on every body (identity swap, occlusion, wrong object: one attempt) and degrades to negative where signatures diverge (pose bias and stale pose: three distinct signatures across four bodies). Same-body memory about *other* faults harms recovery under partial-match retrieval (6.52) and is nearly neutral under exact matching (4.85); the residue is the bias/stale pair, which leaves identical signatures on one body and which only a probe separates. Letting the memory abstain — recommend nothing rather than borrow another fault's repair — lowered the mean and removed the negative-transfer cases but one: the Panda's low lift, whose signature is identical to a different fault's on another body, so even an exact match is a borrowed guess. Abstention is therefore a stated part of the method, and identical signatures across different faults are its known limit. The contract-versus-trace gap (H2) is small — 1.85 against 2.22 — because error text already carries object-level information; a sharper H2 test needs faults whose traces and contract violations disagree.

**Targeted measurement, same day.** Step 4 of §7 was added and scored on the same frozen tables: a passive re-observation and an active bounded nudge (20 mm, then re-observe), chosen by a hand-written rule when the signature is ambiguous between a biased and a stale estimate, with a random-probe baseline. The nudge separates the two on every body where both faults bit (a biased estimate tracks it, a stale one does not); the passive probe separates nothing. Charged as a full attempt, the probe saves nothing — a two-way ambiguity the passive order already tries back to back can save at most one attempt for one spent. Charged at a quarter of an attempt it helps modestly (cross-body memory 1.85 → 1.70), on the four cases where the rule fires. The quantity that decides whether active measurement pays is therefore the cost of a probe relative to a task attempt, which the hardware study must measure; the probe's accuracy is not in question.

What it does not support: any hardware claim, any learned-selector claim, and any claim about faults beyond the eight injected. The RoArm model carries placeholder servo gains. Five of the thirty-two injections caused no failure on their body and were not scored. Per the phase 2b gate, the signal is measurable in simulation for program and perception faults, so the hardware track proceeds with H1 intact; the negative-transfer mechanism is now a stated part of the method to fix (retrieval must weight exact signature matches over partial ones, or abstain).

### Widened benchmark, run (16 September 2026; re-measured 17 September)

The slice was widened along every axis §10 names and re-run (`botcortex/research/`, report `docs/research/benchmark_v2.md`, tables `docs/research/data/v2/`): six bodies, four task families (relational pick-and-place, transport past a bystander on the carry line, a deliberate push with a task-declared allowed-effect set, placement into a 60 mm pocket with a centimetre of clearance), four fault groups (the eight above plus a push waypoint too high, a 20 mm tool offset the controller believes, a 30 mm camera-to-base offset that moves fixtures too, and servo gains at a fifth), twelve one-to-one repairs, and five probes (nudge, re-observe, a known-fixture check, a tool touch-off, a servo step response). The first four bodies are the OpenArm v1, Panda, RoArm-M2 and xArm7; the SO-101 and ViperX 300s were brought up afterwards, specifically to test a claim the four-body run had made, and are the study's only unseen-body evidence. Coverage is measured, not declared: the pocket is declined by the RoArm and failed clean by the xArm7 (7 mm), so neither is scored on it; the push passes on all six bodies only with a program that observes and corrects itself. Faults that took no effect are recorded and not scored. **189 scored cases**, against 117 at four bodies.

**A correction.** The 16 September scoring reported 178 cases and had the ViperX failing the pocket, explained by wrist-servo sag. That explanation was wrong: the pocket had been placed by hand in every workcell and never checked against the other furniture, so it overlapped or crowded the tray on five bodies and hung 36 mm off the table on the ViperX. The layouts were fixed, a test now guards them on every body, and every outcome table was rebuilt from scratch. Outside the pocket task no verdict changed on the body compared row by row; the ViperX gained a scored task. All numbers below are from the rebuild.

| Arm | Mean attempts to verified recovery (189 cases, budget 8) | Within budget | Saved vs no memory | Worse than no memory |
| --- | --- | --- | --- | --- |
| No memory / prior order | 5.00 | 122/189 | 0 | 0/189 |
| Always recalibrate first (dossier baseline 2) | 4.53 | 111/189 | +0.10 | 100/189 |
| Hand-written rules, no memory | 3.19 | 163/189 | +2.21 | 34/189 |
| Target-body memory, other faults only | 2.91 | 170/189 | +2.64 | 39/189 |
| **Cross-body memory, contract signatures, partial matches allowed** | **2.84** | **167/189** | **+2.62** | **13/189** |
| Cross-body memory, exact match or abstain | 2.93 | 165/189 | +2.49 | 13/189 |
| Cross-body memory, trace text (H2 control) | 2.97 | 161/189 | +2.35 | 22/189 |
| Cross-body memory plus the rule probe, probe at unit cost / quarter cost | 2.77 / 2.37 | 174/189 | +2.88 / +3.28 | 32/189 |

**The generalisation split.** The rules were hand-written after reading the fault catalog and the first four bodies' tables; the SO-101 and the ViperX arrived later and the rules were never re-tuned for them. Splitting the same run on that line is the cleanest generalisation test the benchmark contains (means with censored cases counted at the budget, so the halves compare):

| Arm | Four bodies the rules were written against (n=119) | Two bodies neither had seen (n=70) | Degradation |
| --- | --- | --- | --- |
| No memory / prior order | 6.08 | 6.04 | — |
| Hand-written rules | 3.34 | 4.73 | +1.39 |
| Target-body memory | 3.24 | 3.74 | +0.51 |
| **Cross-body memory** | **3.18** | **3.89** | **+0.70** |
| Cross-body memory, exact or abstain | 3.36 | 3.94 | +0.58 |

On the bodies the rules were tuned against they are close to memory (3.34 against 3.18); on the two neither had seen the rules lose 1.39 attempts and memory 0.70, and memory wins by 0.84. The no-memory row is what lets that be read as a property of the arms rather than of the bodies: the prior order needs 6.08 attempts on the four and 6.04 on the two, so the unseen bodies are not harder. Transferred repair knowledge degrades about half as fast as hand-written rules when the body is new. This is the claim the four-body run could not make, it survived a from-scratch rebuild with a wider margin, and it is the strongest form of H1 the study currently has. It is a post-hoc split of one run rather than a pre-registered held-out set; what makes it meaningful is that the rules were never revised after the two bodies arrived.

What changed against the pilot. H1 holds at six bodies, four tasks and four fault groups, with the fewest negative cases of any arm (13 of 189, against the rules' 34). The abstention result **reversed** and stayed reversed: exact-match-or-abstain, the pilot's best arm, is behind loose retrieval (2.93 against 2.84), because with twelve repairs the prior order a silent memory falls back to is long. An earlier scoring suggested abstention generalised much better to unseen bodies; on the rebuilt tables the difference is small (+0.58 against +0.70) and is not claimed. The H2 gap stays small (2.84 against 2.97) for the same reason as before. Probes pay only when cheap — at a quarter of an attempt the probing memory arm is the best in the study at 2.37 — but the new fault pairs need them: the fixture check is what separates a frame offset from a biased estimator, and random probing at the same trigger is consistently worse than the rule, so the choice of measurement carries the value.

**A prediction the wider benchmark falsified.** The four-body run found that loose cross-body memory never recovered a carry too low, concluded the cause was data sparsity — no other body had recorded one on the same task — and stated that the fix was more bodies rather than cleverer retrieval. The SO-101 and the ViperX were added. The sparsity is gone: from the SO-101's side, memory holds 154 records of the repair across five bodies, 9 of them recoveries. Loose retrieval now recovers 5 of the 12 cases where the fault bites and exact-match 7, while the hand-written rules recover all 12 in 1.25 attempts. More bodies helped exactly where the prediction said and not where it mattered, and the real cause is aliasing. The same injected fault presents as a collision with the target still held (Panda, OpenArm at the pocket), a lost grasp with the target still held (ViperX), or a collision with nothing held (SO-101, whose printed jaw never closes at the lowered approach). Where the jaws still hold the target the signature is distinctive and memory transfers the repair — all five of loose retrieval's recoveries. Where the arm never closed on the block, the failure is signature-identical to weak servo gains, and of the 216 records sharing that exact key, retuning gains recovered 8 of its 18 and raising the lift 0 of its 18. Memory reports the data correctly and is wrong about the case, on all three SO-101 tasks; more records make it more confidently wrong. What fixes it is a measurement: the rules ask the servo step response, while memory with the same probe available recovers the same five and no more, because the probe enters retrieval as a filter on candidate records rather than as a discriminator between two hypotheses the signature cannot separate. That is the next piece of machinery this line of work needs — a retrieval that can tell when its own signature is ambiguous and spend a probe to break the tie instead of averaging over an alias — and it is a sharper research question than the one the four-body run posed.
...
A starting decision rule can rank candidate measurements by expected hypothesis reduction relative to time and execution cost. Compare it with random probes and a simple rule-based choice. Do not label a hand-written heuristic a learned causal policy. The simulation pilot (§2) implemented exactly the hand-written version: one active probe, one passive, a rule for when to spend, and a random baseline. Its lesson is that a probe's value is bounded by the size of the ambiguity it resolves and decided by its cost relative to an attempt — measure that ratio on hardware before designing an information-gain policy around it.
...
The two physical arms anchor the claim; simulated arms give it breadth. **Done as of 16 September 2026:** the vendored OpenArm MuJoCo model, a RoArm-M2-Pro model built from the vendor geometry, and four public arm models from MuJoCo Menagerie — the Franka Panda, the xArm7, the SO-101 and the Trossen ViperX 300s — are each bound through the same platform descriptor and conformance suite, six bodies in all. [36](#ref-36) Menagerie models vary in dynamic fidelity; treat them as distinct kinematic/gripper embodiments for screening program, perception, and frame faults, and do not read simulated contact/physics transfer as a hardware result.
...
| BotCortex is already a hardware-portable SDK. | Not established; six simulated bodies now run through one descriptor, no real adapter yet. | Two implemented real adapters, sensing, binding, and conformance evidence. |
...
| Repair knowledge transfers across robot bodies (H1). | Supported in simulation only: 4.70 → 1.59 attempts on held-out bodies with exact-match-or-abstain retrieval, 1/27 negative-transfer case; 1.85 with partial matching (15 Sep 2026 pilot, re-scored on the vendor-geometry RoArm). | Physical results on both arms; more task families and faults; a targeted-measurement step to separate bias from stale pose. |
| The transfer depends on the contract representation (H2). | Weakly supported in simulation: contract 1.85 vs trace 2.22, right family 25/27 vs 19/27. | Faults whose traces and contract violations disagree; a trace arm with agent-written lessons. |
