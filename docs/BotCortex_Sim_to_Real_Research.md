...
The slice was widened along every axis §10 names and re-run (`botcortex/research/`, report `docs/research/benchmark_v2.md`, tables `docs/research/data/v2/`): four task families (relational pick-and-place, transport past a bystander on the carry line, a deliberate push with a task-declared allowed-effect set, placement into a 60 mm pocket with a centimetre of clearance), four fault groups (the eight above plus a push waypoint too high, a 20 mm tool offset the controller believes, a 30 mm camera-to-base offset that moves fixtures too, and servo gains at a fifth), twelve one-to-one repairs, and five probes (nudge, re-observe, a known-fixture check, a tool touch-off, a servo step response). Coverage is measured, not declared: the pocket is declined by the RoArm and failed clean by the xArm7 (12 mm arrival error), so neither is scored on it; the push passes on all four bodies only with a program that observes and corrects itself. Faults that took no effect are recorded and not scored, and the pattern is itself a result: a 30 mm bias or frame offset does not break the OpenArm's pick but breaks its pocket placement; the tool offset breaks no pick anywhere, only the pocket. **117 scored cases.**

| Arm | Mean attempts to verified recovery (117 cases, budget 8) | Within budget | Saved vs no memory | Worse than no memory |
| --- | --- | --- | --- | --- |
| No memory / prior order | 5.12 | 73/117 | 0 | 0/117 |
| Always recalibrate first (dossier baseline 2) | 4.51 | 68/117 | +0.23 | 59/117 |
| Hand-written rules, no memory | 2.79 | 103/117 | +2.79 | 12/117 |
| Target-body memory, other faults only | 2.87 | 108/117 | +2.94 | 22/117 |
| **Cross-body memory, contract signatures, partial matches allowed** | **2.75** | **110/117** | **+3.14** | **9/117** |
| Cross-body memory, exact match or abstain | 2.98 | 102/117 | +2.58 | 9/117 |
| Cross-body memory, trace text (H2 control) | 3.01 | 110/117 | +2.90 | 16/117 |
| Cross-body memory plus the rule probe, probe at unit cost / quarter cost | 2.45 / 2.06 | 109/117 | +3.38 / +3.74 | 16/117 |

What changed against the pilot. H1 holds at four tasks and four fault groups, with the fewest negative cases of any memory arm. The abstention result **reversed**: exact-match-or-abstain, the pilot's best arm, is now the worst memory arm, because with twelve repairs the prior order a silent memory falls back to is long; whether abstention pays depends on the size of the repair catalog, and the paper will report that dependence rather than a fixed threshold. The H2 gap stays small (2.75 against 3.01) for the same reason as before. Probes pay only when cheap — at a quarter of an attempt the probing memory arm is the best in the study — but the new fault pairs need them: the fixture check is what separates a frame offset from a biased estimator (memory alone spends 4.5 attempts on the frame offset, the rules that ask the fixture 2.0), and random probing at the same trigger is consistently worse than the rule, so the choice of measurement carries the value. One fault stays out of memory's reach on every body: a carry too low. The rules were fixed for it (a collision with the block still where it was is a carry too low; the servo probe rules out weak gains) and recover in 1.0 attempts, and the evidence packet now records what the jaws held when the attempt ended (target, another object, nothing), which took the rules from 3.08 to 2.79 overall and lets exact-match memory recover the low carry on 3 of 4 cases. Loose cross-body memory still exhausts its budget on all four, and the tables say why: on every other body the low carry took no effect on the same task (the runtime's routing lifts over the bystander), so no body ever recorded one, and the nearest records — drops too deep — recommend the wrong height. Memory transfers what some body recorded; a fault that bites on one body only has nothing to transfer. That argues for more bodies, not a cleverer retrieval, and the two next bodies (SO-101, ViperX 300s) are in progress.
