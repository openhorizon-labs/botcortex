...
**Widened benchmark — 16 September 2026:** the slice was widened to six bodies, four task families, four fault groups, twelve repairs and five probes (§2, *Widened benchmark*; report `botcortex-runtime/docs/research/benchmark_v2.md`). 178 scored cases. Cross-body repair memory recovers in 2.86 attempts against 5.05 with no memory and 3.14 for hand-written rules, with the fewest negative-transfer cases of any arm (14 of 178). The sharpest result is the split between the four bodies the rules were written against and the two brought up afterwards: the rules lose 1.13 attempts on the unseen bodies and memory 0.61. A prediction the four-body run made — that more bodies would let memory learn a carry too low — was tested by adding those bodies and failed; the cause is signature aliasing across gripper types, not data sparsity. Not a hardware result.
...
### Widened benchmark, run (16 September 2026)

The slice was widened along every axis §10 names and re-run (`botcortex/research/`, report `docs/research/benchmark_v2.md`, tables `docs/research/data/v2/`): six bodies, four task families (relational pick-and-place, transport past a bystander on the carry line, a deliberate push with a task-declared allowed-effect set, placement into a 60 mm pocket with a centimetre of clearance), four fault groups (the eight above plus a push waypoint too high, a 20 mm tool offset the controller believes, a 30 mm camera-to-base offset that moves fixtures too, and servo gains at a fifth), twelve one-to-one repairs, and five probes (nudge, re-observe, a known-fixture check, a tool touch-off, a servo step response). The first four bodies are the OpenArm v1, Panda, RoArm-M2 and xArm7; the SO-101 and ViperX 300s were brought up afterwards, specifically to test a claim the four-body run had made, and are the study's only unseen-body evidence. Coverage is measured, not declared: the pocket is declined by the RoArm and failed clean by the xArm7 (9 mm) and the ViperX (21 mm), so none of the three is scored on it; the push passes on all six bodies only with a program that observes and corrects itself. Faults that took no effect are recorded and not scored, and the pattern is itself a result: a 30 mm bias or frame offset does not break the OpenArm's pick but breaks its pocket placement; the tool offset breaks no pick anywhere, only the pocket. **178 scored cases**, against 117 at four bodies.

| Arm | Mean attempts to verified recovery (178 cases, budget 8) | Within budget | Saved vs no memory | Worse than no memory |
| --- | --- | --- | --- | --- |
| No memory / prior order | 5.05 | 115/178 | 0 | 0/178 |
| Always recalibrate first (dossier baseline 2) | 4.54 | 103/178 | +0.10 | 94/178 |
| Hand-written rules, no memory | 3.14 | 154/178 | +2.30 | 31/178 |
| Target-body memory, other faults only | 3.04 | 162/178 | +2.61 | 39/178 |
| **Cross-body memory, contract signatures, partial matches allowed** | **2.86** | **154/178** | **+2.54** | **14/178** |
| Cross-body memory, exact match or abstain | 2.97 | 152/178 | +2.39 | 13/178 |
| Cross-body memory, trace text (H2 control) | 3.00 | 153/178 | +2.39 | 19/178 |
| Cross-body memory plus the rule probe, probe at unit cost / quarter cost | 2.77 / 2.33 | 159/178 | +2.76 | 30/178 |
...
| Arm | Four bodies the rules were written against (n=117) | Two bodies neither had seen (n=61) | Degradation |
| --- | --- | --- | --- |
| No memory / prior order | 6.21 | 5.89 | — |
| Hand-written rules | 3.41 | 4.54 | +1.13 |
| Target-body memory | 3.26 | 3.92 | +0.66 |
| **Cross-body memory** | **3.34** | **3.95** | **+0.61** |
| Cross-body memory, exact or abstain | 3.64 | 3.84 | +0.20 |

On the bodies the rules were tuned against they tie with memory (3.41 against 3.34); on the two neither had seen the rules lose 1.13 attempts and memory 0.61, and memory wins by 0.59. The no-memory row is what lets that be read as a property of the arms rather than of the bodies: the prior order needs 6.21 attempts on the four and 5.89 on the two, so the unseen bodies are not harder, and the rules lose more than an attempt on cases that are if anything slightly easier. Transferred repair knowledge degrades about half as fast as hand-written rules when the body is new. This is the claim the four-body run could not make, because there the rules matched memory overall, and it is the strongest form of H1 the study currently has. It is a post-hoc split of one run rather than a pre-registered held-out set; what makes it meaningful is that the rules were never revised after the two bodies arrived.

What changed against the pilot. H1 holds at six bodies, four tasks and four fault groups, with the fewest negative cases of any arm (14 of 178, against the rules' 31). The abstention result **reversed** and stayed reversed: exact-match-or-abstain, the pilot's best arm, is behind loose retrieval overall (2.97 against 2.86), because with twelve repairs the prior order a silent memory falls back to is long. Its one advantage is generalisation — it degrades by 0.20 on the unseen bodies against loose retrieval's 0.61, because it never borrows a neighbouring body's wrong majority — so whether abstention pays depends on the size of the repair catalog and on how unlike the new body is, and the paper will report that dependence rather than a fixed threshold. The H2 gap stays small (2.86 against 3.00) for the same reason as before. Probes pay only when cheap — at a quarter of an attempt the probing memory arm is the best in the study at 2.33 — but the new fault pairs need them: the fixture check is what separates a frame offset from a biased estimator, and random probing at the same trigger is consistently worse than the rule, so the choice of measurement carries the value.

**A prediction the wider benchmark falsified.** The four-body run found that loose cross-body memory never recovered a carry too low, concluded the cause was data sparsity — no other body had recorded one on the same task — and stated that the fix was more bodies rather than cleverer retrieval. The SO-101 and the ViperX were added and the prediction failed. The sparsity is gone: memory now holds 143 records of the repair across five bodies and four tasks, 7 of them recoveries. Loose retrieval still recovers **0 of 10**, and exact-match retrieval recovers 5 of 10, a lower rate than the 3 of 4 it managed on less evidence. The real cause is aliasing. The same injected fault presents as a collision with the target still held (Panda), a lost grasp with the target still held (ViperX), or a collision with nothing held (SO-101, whose printed jaw never closes at the lowered approach). Where the jaws still hold the target the signature is distinctive and memory transfers the repair — four of exact match's five recoveries. Where the arm never closed on the block, the failure is signature-identical to weak servo gains, and of the 180 records sharing that exact key, retuning gains recovered 7 of its 15 and raising the lift 0 of its 15. Memory reports the data correctly and is wrong about the case; more records make it more confidently wrong. What fixes it is a measurement: the rules ask the servo step response and recover all ten in 1.0 attempts, while memory with the same probe available recovers none, because the probe enters retrieval as a filter on candidate records rather than as a discriminator between two hypotheses the signature cannot separate. That is the next piece of machinery this line of work needs — a retrieval that can tell when its own signature is ambiguous and spend a probe to break the tie instead of averaging over an alias — and it is a sharper research question than the one the four-body run posed.
