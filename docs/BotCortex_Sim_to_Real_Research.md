...
**Simulation pilot — 15 September 2026:** the thin slice (§15) ran end to end in simulation on four bodies. Cross-body repair memory cut attempts-to-recovery from 4.61 to 2.00 on a held-out body and beat hand-written rules (2.57); negative transfer was measured (5 of 28 cases) and then removed by letting retrieval abstain on partial matches (1.50 attempts, 0 of 28 worse than no memory); same-body memory about other faults was harmful (6.61). Details in §2 under *Simulation pilot* and in `botcortex-runtime/docs/research/slice_pilot.md`. Not a hardware result.
...
| Cross-body memory, contract signatures, partial matches allowed | 2.00 | +2.61 | 5/28 | 26/28 |
| **Cross-body memory, contract signatures, exact match or abstain** | **1.50** | **+3.11** | **0/28** | 26/28 |
| Cross-body memory, exact, plus the rule probe at a quarter cost | 1.43 | +3.18 | 0/28 | 26/28 |
| Cross-body memory, trace text (H2 control) | 2.32 | +2.29 | 2/28 | 21/28 |

What the pilot supports, and only in simulation: repair knowledge from other bodies reduces recovery attempts on an unseen body (H1 direction), beating the rule baseline; transfer is perfect where a fault leaves the same contract signature on every body (identity swap, occlusion, wrong object: one attempt) and degrades to negative where signatures diverge (pose bias and stale pose: three distinct signatures across four bodies). Same-body memory about *other* faults harms recovery under partial-match retrieval (6.61) and is nearly neutral under exact matching (4.75); the residue is the bias/stale pair, which leaves identical signatures on one body and which only a probe separates. Letting the memory abstain — recommend nothing rather than borrow another fault's repair — removed every negative-transfer case and lowered the mean, which makes abstention a stated part of the method rather than a hoped-for property. The contract-versus-trace gap (H2) is small — 2.00 against 2.32 — because error text already carries object-level information; a sharper H2 test needs faults whose traces and contract violations disagree.

**Targeted measurement, same day.** Step 4 of §7 was added and scored on the same frozen tables: a passive re-observation and an active bounded nudge (20 mm, then re-observe), chosen by a hand-written rule when the signature is ambiguous between a biased and a stale estimate, with a random-probe baseline. The nudge separates the two on every body where both faults bit (a biased estimate tracks it, a stale one does not); the passive probe separates nothing. Charged as a full attempt, the probe saves nothing — a two-way ambiguity the passive order already tries back to back can save at most one attempt for one spent. Charged at a quarter of an attempt it helps modestly (cross-body memory 2.00 → 1.89), on the four cases where the rule fires. The quantity that decides whether active measurement pays is therefore the cost of a probe relative to a task attempt, which the hardware study must measure; the probe's accuracy is not in question.
...
| Repair knowledge transfers across robot bodies (H1). | Supported in simulation only: 4.61 → 1.50 attempts on held-out bodies with exact-match-or-abstain retrieval, 0/28 negative-transfer cases; 2.00 with partial matching, 5/28 negative (15 Sep 2026 pilot). | Physical results on both arms; more task families and faults; a targeted-measurement step to separate bias from stale pose. |
